    ...
    <div className='flex flex-col place-items-center'>
      <div className='flex flex-row gap-2 mx-10 my-3 self-start flex-wrap'>
        <p className='hidden lg:block'>filter:</p>
          ...
          <p key={category} onClick={() => toggleCategory(category)} className={`${getColorForCategory(category)} lg:px-3 px-2 rounded-md hover:cursor-pointer border-2 lg:text-normal text-sm border-gray-300/15 text-white transition-all lg:hover:shadow-[inset_0_0px_5px_rgba(0,0,0,0.6)] lg:hover:shadow-neutral-700/25 lg:hover:border-neutral-700/25 lg:outline outline-1 hover:outline-gray-gray-800`} >
              {`${category}`}
          </p>
          ))}
      </div>
